...
import pandas as pd
from models.model import recommend
import os
import re
...
def load_data():
    ...
    fp_growth_rules = pd.read_pickle(os.path.join(data_folder_path,'fp_growth_rules.pkl'))
...
if st.button("Get Recommendations"):
    if algorithm == 'apriori':
        recommendations = recommend(apriori_rules, selected_ids)
    else:
        recommendations = recommend(fp_growth_rules, selected_ids)
    if recommendations:
